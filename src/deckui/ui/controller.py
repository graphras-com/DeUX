...
A *controller* in DeckUI is the small bridge between a backend service
(audio player, smart bulb, timer, etc.) and a :class:`~deckui.DuiCard`
or :class:`~deckui.DuiKey`.  It loads a ``.dui`` package, wires
service-event subscriptions to bindings (typically via
:meth:`~deckui.DuiCard.bind` / :meth:`~deckui.DuiKey.bind`), and
